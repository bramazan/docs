...
Webhook'lar; transferin başarılı tamamlanma, asenkron kabul (delivered) ve başarısızlık olaylarını **sunucunuza HTTP POST** ile gönderir. EFT/FAST işlemlerinin **asenkron** yapısı nedeniyle webhook entegrasyonu pratikte zorunludur — `processing` veya `delivered` durumdaki transferin son sonucunu webhook ile yakalarsınız.
...
X-Payven-Event:        transfer.completed
...
Gövde olayın kendisidir (**düz/flat** JSON — ayrıca `data` zarfı yoktur):
...
async function handleTransferWebhook(req, res) {
  ...
  if (eventType === "transfer.completed" || eventType === "transfer.delivered") {
