...
flowchart LR
    ...
    D --> E[transfer.completed<br/>veya .failed webhook]
```
...
Tekrarlayan kayıt **kendisi** olay yayınlamaz; ancak ürettiği her transfer terminal sonuca ulaştığında standart `transfer.completed` / `.delivered` / `.failed` olayları yayınlanır. Üretilen transferi tekrarlayan kayda bağlamak için transferin `external_id`'sini kendi tarafınızda eşleyin.
