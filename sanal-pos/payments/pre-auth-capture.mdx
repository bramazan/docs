...
flowchart LR
    ...
    B -->|POST .../void| D[canceled]
```

`completed` durumundan sonra mutabakata dahil etme süreci ayrı `Settlement` kaynağında izlenir — bkz. [Settlement Kayıtları](/sanal-pos/reporting/settlements).
...
|---|---|---|---|
| `amount.amount` | int (kuruş) | Opsiyonel | Çekilecek tutar. Boş veya `0` gönderilirse **kalan tüm rezerve tutar** çekilir. |
| `amount.currency` | string | Opsiyonel | Para birimi (boşsa orijinal işlemin para birimi) |
| `reason` | string | Opsiyonel | Capture sebebi (raporlama için) |
...
```json
{
  ...
  "status":         "completed",
  "extra_properties": {
    "processed_at":    "2026-05-03T18:00:01.234+03:00",
    "auth_code":       "789012",
