...
Gövde olayın kendisidir (**düz/flat** JSON — ayrıca bir `data` zarfı yoktur). Olay tipi ve
teslimat metadata'sı HTTP header'larında taşınır:
...
```json
{
  ...
  "max_retry_count": 5,
  "is_active":       true,
  "created_at":      "2026-05-03T12:34:56.789+03:00"
}
```

<Warning>
**`secret` değerini saklayın** — yalnızca bu yanıtta bir kez gösterilir. İmza
doğrulamasında bu değeri kullanacaksınız. Kaybedilirse `POST /webhook-subscriptions/{id}/rotate-secret`
...
<CardGroup cols={2}>
  ...
  <Card title="Olay Tipleri" icon="list" href="/sanal-pos/webhooks/events">
    Tüm 31 olay tipi ve payload alanları.
  </Card>
